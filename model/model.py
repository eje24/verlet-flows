    ...
    def forward(self, x):
        ...
        if self.lm_embedding_type is not None:
            assert x.shape[1] == self.num_categorical_features + \
                self.num_scalar_features + self.lm_embedding_dim
        else:
            assert x.shape[1] == self.num_categorical_features + \
...
class TensorProductScoreModel(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        for l in range(len(self.lig_conv_layers)):
            ...
            lig_edge_attr_ = torch.cat(
                [lig_edge_attr, lig_node_attr[lig_src, :self.ns], lig_node_attr[lig_dst, :self.ns]], -1)
            ...
            rec_to_lig_edge_attr_ = torch.cat(
                [cross_edge_attr, lig_node_attr[cross_lig, :self.ns], rec_node_attr[cross_rec, :self.ns]], -1)
            ...
            if l != len(self.lig_conv_layers) - 1:
                rec_edge_attr_ = torch.cat(
                    [rec_edge_attr, rec_node_attr[rec_src, :self.ns], rec_node_attr[rec_dst, :self.ns]], -1)
    ...
    def build_lig_conv_graph(self, data):
        ...
        edge_index = torch.cat(
            [data['ligand', 'ligand'].edge_index, radius_edges], 1).long()
        edge_attr = torch.cat([
            data['ligand', 'ligand'].edge_attr,
            torch.zeros(
                radius_edges.shape[-1], self.in_lig_edge_features, device=data['ligand'].x.device)
        ], 0)
        ...
        node_attr = torch.cat(
            [data['ligand'].x, data['ligand'].node_sigma_emb], 1)
    ...
    def build_rec_conv_graph(self, data):
        ...
        src, dst = edge_index
        edge_vec = data['receptor'].pos[dst.long()] - \
            data['receptor'].pos[src.long()]
    ...
    def build_cross_conv_graph(self, data, cross_distance_cutoff):
        ...
        if torch.is_tensor(cross_distance_cutoff):
            ...
            edge_index = radius(data['receptor'].pos / cross_distance_cutoff[data['receptor'].batch],
                                data['ligand'].pos /
                                cross_distance_cutoff[data['ligand'].batch], 1,
    ...
    def build_center_conv_graph(self, data):
        ...
        edge_index = torch.cat([data['ligand'].batch.unsqueeze(0), torch.arange(
            len(data['ligand'].batch)).to(data['ligand'].x.device).unsqueeze(0)], dim=0)

        center_pos, count = torch.zeros((data.num_graphs, 3)).to(
            data['ligand'].x.device), torch.zeros((data.num_graphs, 3)).to(data['ligand'].x.device)
    ...
    def build_bond_conv_graph(self, data):
        ...
        bonds = data['ligand', 'ligand'].edge_index[:,
                                                    data['ligand'].edge_mask].long()
