...
def hutch_trace(x_out, x_in, noise=None, **kwargs):
    ...
    # Cleave disregard time dimension
    noise[:,0] = 0.0
    jvp = grad(x_out, x_in, noise, create_graph=True, allow_unused=True)[0]
    trJ = torch.einsum('bi,bi->b', jvp, noise)
    # print(f'Indices more than 100 {torch.nonzero(torch.abs(trJ) > 25.0)}')
    return trJ

REQUIRES_NOISE = [hutch_trace]

class CNF(nn.Module):
    def __init__(self, net:nn.Module, trace_estimator:Union[Callable, None]=None, noise_dist=None, order=1, verbose=False):
    ...
    def forward(self, x):
        with torch.set_grad_enabled(True):
            ...
            t = x[:, 1][0].detach().item()
            if self.verbose:
                print(f'trJ at t={t:.3f}: mean is {trJ.mean():.3f}, max is {trJ.max():.3f}, min is {trJ.min():.3f}, var is {trJ.var():.3f} and mean cumulative log J: {x[:, 0].mean():.3f}')
