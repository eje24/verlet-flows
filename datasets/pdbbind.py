# Based on https://github.com/gcorso/DiffDock/blob/main/datasets/pdbbind.py
import binascii
import glob
import hashlib
import os
...
class PDBBind(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0,
        ...
        self.full_cache_path = os.path.join(cache_path, f'limit{self.limit_complexes}'
                                            ...
                                            + ('' if not matching or num_conformers ==
                                               1 else f'_confs{num_conformers}')
        ...
        print('loading data from memory: ', os.path.join(
            self.full_cache_path, "heterographs.pkl"))
    ...
    def preprocessing(self):
        ...
        if self.esm_embeddings_path is not None:
            ...
            for name in complex_names_all:
                lm_embeddings_chains_all.append(
                    chain_embeddings_dictlist[name])
        else:
        ...
        if self.num_workers > 1:
            ...
            for i in range(len(complex_names_all)//1000+1):
                ...
                lm_embeddings_chains = lm_embeddings_chains_all[1000*i:1000*(
                    i+1)]
    ...
    def inference_preprocessing(self):
        ...
        if self.esm_embeddings_path is not None:
            ...
            for protein_path in self.protein_path_list:
                embeddings_paths = sorted(glob.glob(os.path.join(
                    self.esm_embeddings_path, os.path.basename(protein_path)) + '*'))
                lm_embeddings_chains = []
                for embeddings_path in embeddings_paths:
                    lm_embeddings_chains.append(torch.load(
                        embeddings_path)['representations'][33])
        ...
        if self.num_workers > 1:
            ...
            for i in range(len(self.protein_path_list)//1000+1):
                ...
                ligand_description_chunk = self.ligand_descriptions[1000*i:1000*(
                    i+1)]
            ...
            if complex_graphs == []:
                raise Exception(
                    'Preprocessing did not succeed for any complex')
            with open(os.path.join(self.full_cache_path, "heterographs.pkl"), 'wb') as f:
                pickle.dump((complex_graphs), f)
            with open(os.path.join(self.full_cache_path, "rdkit_ligands.pkl"), 'wb') as f:
                pickle.dump((rdkit_ligands), f)
...
def print_statistics(complex_graphs):
    ...
    for complex_graph in complex_graphs:
        lig_pos = complex_graph['ligand'].pos if torch.is_tensor(
            complex_graph['ligand'].pos) else complex_graph['ligand'].pos[0]
        radius_protein = torch.max(torch.linalg.vector_norm(
            complex_graph['receptor'].pos, dim=1))
    ...
    name = ['radius protein', 'radius molecule',
            'distance protein-mol', 'rmsd matching']
...
def construct_loader(args):
    ...
    val_dataset = PDBBind(cache_path=args.cache_path,
                          split_path=args.split_val, keep_original=True, **common_args)
...
def read_mol(pdbbind_dir, name, remove_hs=False):
    lig = read_molecule(os.path.join(
        pdbbind_dir, name, f'{name}_ligand.sdf'), remove_hs=remove_hs, sanitize=True)
    if lig is None:  # read mol2 file if sdf file cannot be sanitized
        print('Using the .sdf file failed. We found a .mol2 file instead and are trying to use that.')
        lig = read_molecule(os.path.join(
            pdbbind_dir, name, f'{name}_ligand.mol2'), remove_hs=remove_hs, sanitize=True)
    return lig
