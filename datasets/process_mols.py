# Based on https://github.com/gcorso/DiffDock/blob/main/datasets/process_mols.py
import copy
import os
import warnings
...
def lig_atom_featurizer(mol):
    ...
    for idx, atom in enumerate(mol.GetAtoms()):
        atom_features_list.append([
            ...
            safe_index(
                allowable_features['possible_implicit_valence_list'], atom.GetImplicitValence()),
            safe_index(
...
def rec_residue_featurizer(rec):
    ...
    for residue in rec.get_residues():
        feature_list.append(
            [safe_index(allowable_features['possible_amino_acids'], residue.get_resname())])
...
def extract_receptor_structure(rec, lig, lm_embedding_chains=None):
    ...
    lm_embeddings = np.concatenate(
        valid_lm_embeddings, axis=0) if lm_embedding_chains is not None else None
...
def get_lig_graph(mol, complex_graph):
    ...
    for bond in mol.GetBonds():
        ...
        col += [end, start]
        edge_type += 2 * \
            [bonds[bond.GetBondType()]] if bond.GetBondType(
            ) != BT.UNSPECIFIED else [0, 0]
...
def get_lig_graph_with_matching(mol_, complex_graph, popsize, maxiter, matching, keep_original, num_conformers, remove_hs):
    if matching:
        ...
        if keep_original:
            complex_graph['ligand'].orig_pos = mol_maybe_noh.GetConformer(
            ).GetPositions()
        ...
        for i in range(num_conformers):
            ...
            if i == 0:
                ...
                complex_graph['ligand'].pos.append(torch.from_numpy(
                    mol_rdkit.GetConformer().GetPositions()).float())
...
def get_calpha_graph(rec, c_alpha_coords, n_coords, c_coords, complex_graph, cutoff=20, max_neighbor=None, lm_embeddings=None):
    ...
    for i in range(num_residues):
        ...
        diff_vecs = c_alpha_coords[src, :] - \
            c_alpha_coords[dst, :]  # (neigh_num, 3)
    ...
    complex_graph['receptor'].x = torch.cat([node_feat, torch.tensor(
        lm_embeddings)], axis=1) if lm_embeddings is not None else node_feat
...
def get_fullrec_graph(rec, rec_coords, c_alpha_coords, n_coords, c_coords, complex_graph, c_alpha_cutoff=20,
    ...
    atom_res_edge_index = torch.from_numpy(np.asarray(
        [np.arange(len(atom_feat)), src_c_alpha_idx])).long()
    ...
    complex_graph['atom', 'atom_rec_contact',
                  'receptor'].edge_index = atom_res_edge_index
